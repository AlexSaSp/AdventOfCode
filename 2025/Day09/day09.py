...
def no_intersections(a, b, coords):
    ...
    for c, d in pairwise(coords):
        if a == (a[0], b[1]) or (a[0], b[1]) == b:
            if intersect(a, b, c, d):
                return False
        elif (
            intersect(a, (a[0], b[1]), c, d)
...
def main():
    ...
    coords = [(int(x.split(',')[0]), int(x.split(',')[1])) for x in lines]
    ...
    for i_idx, i in enumerate(coords):
        for j in coords:
            if i != j:
                ...
                if (
                    area > curr_max # <- For part 1 only this condition is needed
